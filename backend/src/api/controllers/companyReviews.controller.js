...
export const createCompanyReviews = async (req, res) => {
    ...
    try {
        const newcompanyReviews =
            await new companyReviewsService().addCompanyReviews(
                companyReviews.yearOfDrive,
                companyReviews.companyName,
                companyReviews.jobProfile,
...
export const getAllCompanyReviews = async (req, res) => {
    try {
        const companyReviews =
            await new companyReviewsService().getAllCompanyReviews()
...
export const getCompanyReviews = async (req, res) => {
    try {
        const companyReview =
            await new companyReviewsService().getCompanyReviews(
...
export const updateCompanyReviews = async (req, res) => {
    try {
        const companyReviews =
            await new companyReviewsService().updateCompanyReviews(
                req.body,
                req.params.companyReviewsId
            )
        res.status(200).json(companyReviews)
...
export const deleteCompanyReviews = async (req, res) => {
    try {
        await new companyReviewsService().deleteCompanyReviews(
            req.params.companyReviewsId
        )
